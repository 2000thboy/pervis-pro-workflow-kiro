...
| **代码质量** | ⭐⭐⭐⭐⭐ (100%) | 测试覆盖 100%（110/110 通过），技术债务已清理 |
...
| 问题 | 状态 | 说明 |
|------|------|------|
| SQLAlchemy `declarative_base` 警告 | ✅ 已修复 | 8个文件迁移到 `sqlalchemy.orm` |
| Gemini SDK 兼容性 | ✅ 已修复 | 添加新旧 SDK 兼容层 |
| Pydantic V2 警告 | ✅ 无需修复 | 当前配置方式仍受支持 |
| FFmpeg 集成 | ✅ 用户已安装 | 视频渲染功能可用 |
...
│              ▼                   ▼                   ▼                      │
│     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐               │
│     │ ScriptAgent │     │  ArtAgent   │     │ PMAgent     │               │
│     │ (编剧)      │     │  (美术)     │     │ (项目助理)  │               │
│     └─────────────┘     └─────────────┘     └─────────────┘               │
│              │                   │                   │                      │
│              ▼                   ▼                   ▼                      │
│     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐               │
│     │ Storyboard  │     │ MarketAgent │     │ SystemAgent │               │
│     │ Agent       │     │ (市场分析)  │     │ (系统监控)  │               │
│     └─────────────┘     └─────────────┘     └─────────────┘               │
│                                                                             │
│                         ┌─────────────────┐                                 │
...
│       │              │              │              │                       │
│       │              ▼              ▼              ▼                       │
│       │         ScriptAgent    ScriptAgent    ArtAgent                    │
│       │              │              │              │                       │
│       │              └──────────────┴──────────────┘                       │
│       │                             │                                      │
│       │                             ▼                                      │
...
| `project_templates` | 项目模板 | id, name, type, template_data |
| `content_versions` | 版本历史 | id, project_id, version, content |
...
| 优先级 | 问题 | 影响 | 状态 |
|--------|------|------|------|
| ~~P1~~ | ~~SQLAlchemy 2.0 警告~~ | ~~未来兼容性~~ | ✅ 已修复 |
| ~~P1~~ | ~~Gemini SDK 兼容性~~ | ~~API 调用~~ | ✅ 已修复 |
| P2 | httplib2 废弃 API 警告 | 第三方库问题 | 低优先级 |
| P2 | 视觉内核 CLIP 模块缺失 | 降级到 Mock 模式 | 可选安装 |
| P3 | API 文档完善 | 开发体验 | 待处理 |
...
5. **版本管理**: PM_Agent 管理内容版本，支持回滚

### 6.4 架构改进建议

1. **消息总线增强**: 添加消息持久化和重试机制
2. **缓存层**: 引入 Redis 缓存热点数据
3. **异步任务**: 使用 Celery 处理长时间任务
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Phase 1 (已完成)         Phase 2 (2-4周)          Phase 3 (1-2月)         │
│  ─────────────────        ─────────────────        ─────────────────        │
│  技术债务清理 ✅          功能完善                 生产就绪                 │
│  ├─ SQLAlchemy 修复 ✅    ├─ 视频渲染完善          ├─ 用户认证              │
│  ├─ Gemini SDK 修复 ✅    ├─ 前端路由完善          ├─ PostgreSQL            │
│  └─ FFmpeg 集成 ✅        └─ 性能优化              └─ Docker 部署           │
│                                                                             │
...
### 8.2 Phase 1: 技术债务清理 ✅ 已完成

| 迭代项 | 具体内容 | 状态 |
|--------|----------|------|
| **SQLAlchemy 升级** | 迁移 `declarative_base()` 到 `sqlalchemy.orm` | ✅ 已完成 |
| **Gemini SDK 兼容** | 添加新旧 SDK 兼容层 | ✅ 已完成 |
| **FFmpeg 集成** | 用户已安装，视频渲染可用 | ✅ 已完成 |

**修复的文件列表**:
- `backend/models/user.py`
- `backend/models/asset_library.py`
- `backend/models/system_notification.py`
- `backend/models/background_task.py`
- `backend/migrations/006_add_wizard_models.py`
- `backend/migrations/001_add_tag_management.py`
- `backend/migrations/003_add_image_tables.py`
...
```
任务 2.1: 渲染流程完善
├── 2.1.1 实现 FFmpeg 命令生成器
├── 2.1.2 添加渲染进度回调
├── 2.1.3 支持 MP4/MOV/WebM 输出
├── 2.1.4 添加渲染质量选项 (720p/1080p/4K)
└── 2.1.5 实现渲染任务队列
...
| FFmpeg 兼容性问题 | 低 | 中 | 提供预编译版本、详细安装指南 |
...
- ✅ **技术债务**: Phase 1 已全部清理完成
- ⚠️ **待完善**: 视频渲染需优化、CLIP 模块可选安装

**建议**: 
1. ✅ Phase 1 已完成，可进入 Phase 2
...
| 优先级 | 行动项 | 负责人 | 预计时间 |
|--------|--------|--------|----------|
| P0 | 视频渲染流程完善 | 后端开发 | 1 周 |
| P1 | 前端路由系统 | 前端开发 | 1 周 |
| P1 | 内部测试准备 | 产品 | 1 周 |
| P2 | API 文档完善 | 后端开发 | 3 天 |
| P2 | 用户引导设计 | 产品/设计 | 1 周 |
...
| `/api/wizard/parse-script` | POST | 解析剧本 |
| `/api/wizard/generate-content` | POST | AI 生成内容 |
| `/api/wizard/analyze-images` | POST | 视觉分析 |
...
2. `backend/models/asset_library.py`
3. `backend/models/system_notification.py`
4. `backend/models/background_task.py`
5. `backend/migrations/006_add_wizard_models.py`
6. `backend/migrations/001_add_tag_management.py`
7. `backend/migrations/003_add_image_tables.py`
8. `backend/app/database.py`
9. `backend/app/models/base.py`

**Gemini SDK 兼容 (2 个文件)**:
1. `backend/services/gemini_client.py`
2. `backend/services/video_preprocessor.py`

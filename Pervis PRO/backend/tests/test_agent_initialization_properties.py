...
迁移自: multi-agent-workflow/backend/tests/test_agent_initialization_properties.py
"""
import pytest
import asyncio
...
# 更新导入路径以匹配 Pervis PRO 结构
from core.message_bus import MessageBus, Message, MessageType
from core.base_agent import BaseAgent, AgentLifecycleState, AgentOperationLog
from core.agent_types import AgentState, AgentType
from core.communication_protocol import ProtocolMessage, ProtocolMessageType


# 创建一个具体的Agent实现用于测试
class ConcreteTestAgent(BaseAgent):
    """可测试的Agent实现 - 命名避免与pytest收集冲突"""
    
    def __init__(self, agent_id: str, agent_type: AgentType, message_bus_instance: MessageBus, **kwargs):
        super().__init__(agent_id, agent_type, message_bus_instance, **kwargs)
...
# 自定义策略：生成有效的Agent ID（使用显式字符集避免 Unicode 问题）
...
config_strategy = st.dictionaries(
    ...
    values=st.one_of(
        st.integers(),
        st.text(alphabet='abcdefghijklmnopqrstuvwxyz0123456789 ', max_size=50),
        st.booleans()
    ),
    min_size=0,
    max_size=5
    ...
    *对于任何*系统启动操作，所有8个Agent（导演、系统、DAM、PM、编剧、美术、市场、后端）
    ...
    async def test_multiple_agents_initialization(
        ...
        try:
            ...
            # 验证属性5: 消息总线统计正确
            stats = bus.get_stats()
            # Pervis PRO 的 BaseAgent 每个订阅:
            # - 专属主题 (agent.{agent_id})
            # - 广播主题 (agent.broadcast)
            # - 协议专属主题
            # - 协议广播主题
            # 所以每个Agent实际上有4个订阅
            expected_subscriptions = len(agents) * 4

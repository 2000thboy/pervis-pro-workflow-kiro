...
| 标签类型 | 字段名 | 可选值 | 说明 |
|---------|--------|--------|------|
| 音频类型 | `audio_type` | BGM/SFX/VO/AMBIENT/FOLEY | 背景音乐/音效/对白/环境音/拟音 |
| BGM 风格 | `bgm_style` | ORCHESTRAL/ELECTRONIC/ROCK/JAZZ/FOLK/CLASSICAL/POP | 管弦乐/电子/摇滚/爵士/民谣/古典/流行 |
| 情绪 | `audio_mood` | TENSE/SAD/HAPPY/CALM/HORROR/ROMANTIC/EPIC/MYSTERIOUS | 紧张/悲伤/欢乐/平静/恐怖/浪漫/热血/神秘 |
| 节奏 | `tempo` | SLOW/MEDIUM/FAST/VARIABLE | 慢速/中速/快速/变化 |
| BPM 范围 | `bpm_range` | 60-80/80-100/100-120/120-140/140+ | 每分钟节拍数范围 |
| 乐器 | `instruments` | ["钢琴", "小提琴", "吉他", "鼓"] | 主要乐器列表 |
| 场景适用 | `scene_fit` | ACTION/DIALOGUE/TRANSITION/MONTAGE/CLIMAX | 动作/对话/转场/蒙太奇/高潮 |

### 9.2 音频向量嵌入

| 需求 | 规格 |
|------|------|
| 嵌入模型 | 音频特征提取模型（如 CLAP、AudioCLIP）|
| 向量维度 | 512-768 维 |
| 特征提取 | 梅尔频谱、MFCC、节奏特征 |
| 存储方式 | 与视频向量分开存储 |
...
```
用户查询/场景描述 → 查询解析 → 标签匹配 → 向量搜索 → 结果融合 → 排序返回
```
...
1. WHEN 音频素材上传 THEN THE 预处理管道 SHALL 自动执行：
   - 音频格式转换（统一为 WAV/MP3）
   - 音频特征提取（时长、采样率、声道数）
   - 音频标签生成（AI 分析）
   - 音频向量生成
   - 波形图生成（用于前端显示）
2. THE 预处理管道 SHALL 支持批量处理
3. THE 预处理结果 SHALL 存储到数据库
